...
async function summarizeChunk(chunk, openai, settings) {
	...
	const systemPrompt = (settings.summarySystemPrompt && settings.summarySystemPrompt.trim()) ||
		'You summarize discussion forum threads into concise summaries.';
...
exports.summarizeTopic = async function (tid, openai, settings) {
	...
	const finalSystemPrompt = (settings.summaryFinalSystemPrompt && settings.summaryFinalSystemPrompt.trim()) ||
		'You are an assistant that summarizes forum thread summaries into a single cohesive summary.';
	...
	return stripReasoning(finalResponse.choices[0].message.content);
};
